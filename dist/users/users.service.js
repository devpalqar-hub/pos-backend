...
const ADMIN_ROLES = [prisma_1.UserRole.SUPER_ADMIN, prisma_1.UserRole.OWNER, prisma_1.UserRole.RESTAURANT_ADMIN];
const STAFF_ROLES = [prisma_1.UserRole.WAITER, prisma_1.UserRole.CHEF];
const CREATABLE_BY = {
    [prisma_1.UserRole.SUPER_ADMIN]: [
        prisma_1.UserRole.SUPER_ADMIN,
        prisma_1.UserRole.OWNER,
        prisma_1.UserRole.RESTAURANT_ADMIN,
        prisma_1.UserRole.WAITER,
        prisma_1.UserRole.CHEF,
    ],
    [prisma_1.UserRole.OWNER]: [
        prisma_1.UserRole.RESTAURANT_ADMIN,
        prisma_1.UserRole.WAITER,
    ...
    [prisma_1.UserRole.RESTAURANT_ADMIN]: [prisma_1.UserRole.WAITER, prisma_1.UserRole.CHEF],
    [prisma_1.UserRole.WAITER]: [],
    [prisma_1.UserRole.CHEF]: [],
};
let UsersService = class UsersService {
    ...
    async create(actor, dto) {
        ...
        if (actor.role === prisma_1.UserRole.OWNER) {
            if (!dto.restaurantId) {
        ...
        if (actor.role === prisma_1.UserRole.RESTAURANT_ADMIN) {
        ...
        if ([...STAFF_ROLES, prisma_1.UserRole.RESTAURANT_ADMIN].includes(dto.role) && !dto.restaurantId) {
    ...
    async findAll(actor) {
        ...
        switch (actor.role) {
            ...
            case prisma_1.UserRole.RESTAURANT_ADMIN:
                if (!actor.restaurantId)
                    return [];
                users = await this.prisma.user.findMany({
    ...
    async update(actor, targetId, dto) {
        ...
        if (dto.role !== undefined && actor.role !== prisma_1.UserRole.SUPER_ADMIN) {
            throw new common_1.ForbiddenException('Only Super Admin can change user roles');
        }
        if (dto.isActive !== undefined &&
            ![prisma_1.UserRole.SUPER_ADMIN, prisma_1.UserRole.OWNER].includes(actor.role)) {
    ...
    async getProfile(actor) {
        const user = await this.prisma.user.findUnique({
            ...
            include: {
                restaurant: { select: { id: true, name: true } },
                ownedRestaurants: actor.role === prisma_1.UserRole.OWNER
    ...
    async remove(actor, targetId) {
        ...
        else if (actor.role !== prisma_1.UserRole.SUPER_ADMIN) {
    ...
    assertCanViewUser(actor, target) {
        if (actor.role === prisma_1.UserRole.SUPER_ADMIN)
            return;
