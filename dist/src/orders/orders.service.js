...
let OrdersService = OrdersService_1 = class OrdersService {
    ...
    mapSessionStatusToOrderStatus(status) {
        const map = {
            OPEN: 'New',
            BILLED: 'Preparing',
            PAID: 'Served',
    ...
    async getOrderAnalytics(actor, restaurantId, filters) {
        await this.assertRestaurantAccess(actor, restaurantId);
    ...
    async getOrdersWithAnalytics(actor, restaurantId, filters, page, limit, fetchAll = false) {
        await this.assertRestaurantAccess(actor, restaurantId);
        ...
        const paginated = await (0, pagination_util_1.paginate)({
            prismaModel: this.prisma.orderSession,
            page,
            limit,
            fetchAll,
        ...
        const avgSpendTime = completedSessions.reduce((acc, s) => {
            const minutes = (new Date(s.closedAt).getTime() - new Date(s.createdAt).getTime()) /
                60000;
            return acc + minutes;
        }, 0) / (completedSessions.length || 1);
        const tableIds = Array.from(new Set(sessions
            .map((s) => s.tableId)
        ...
        const orders = sessions.map((s) => ({
            ...
            status: this.mapSessionStatusToOrderStatus(s.status),
        }));
        return {
            analytics: {
                total_revenue: {
    ...
    async getOrderDetails(actor, restaurantId, sessionId) {
        await this.assertRestaurantAccess(actor, restaurantId);
        ...
        const items = session.batches.flatMap(batch => batch.items.map(item => ({
            name: item.menuItem.name,
