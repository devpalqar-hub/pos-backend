...
let RestaurantsService = RestaurantsService_1 = class RestaurantsService {
    ...
    async create(actor, dto) {
        ...
        if (owner.role !== prisma_1.UserRole.OWNER) {
    ...
    async findAll(actor) {
        switch (actor.role) {
            ...
            case prisma_1.UserRole.WAITER:
            case prisma_1.UserRole.CHEF:
    ...
    async update(actor, id, dto) {
        ...
        if (dto.ownerId !== undefined && actor.role !== prisma_1.UserRole.SUPER_ADMIN) {
        ...
        if (dto.ownerId) {
            ...
            if (newOwner.role !== prisma_1.UserRole.OWNER) {
    ...
    async assignStaff(actor, restaurantId, dto) {
        ...
        if (user.role === prisma_1.UserRole.SUPER_ADMIN || user.role === prisma_1.UserRole.OWNER) {
            throw new common_1.BadRequestException(`Cannot assign SUPER_ADMIN or OWNER users to a restaurant as staff`);
        }
        if (actor.role === prisma_1.UserRole.OWNER && restaurant.ownerId !== actor.id) {
    ...
    assertCanViewRestaurant(actor, restaurant) {
        ...
        if (actor.role === prisma_1.UserRole.OWNER && restaurant.ownerId === actor.id)
            return;
        if ([prisma_1.UserRole.RESTAURANT_ADMIN, prisma_1.UserRole.WAITER, prisma_1.UserRole.CHEF].includes(actor.role) &&
    ...
    filterResponse(role, restaurant) {
        if (role === prisma_1.UserRole.SUPER_ADMIN || role === prisma_1.UserRole.OWNER) {
            return restaurant;
        }
